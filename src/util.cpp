...
const char * const BITCOIN_CONF_FILENAME = "want.conf";
const char * const BITCOIN_PID_FILENAME = "wantd.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "want" is a composite category enabling all Want-related debug output
            if(ptrCategory->count(string("want"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\WantCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\WantCore
    // Mac: ~/Library/Application Support/WantCore
    // Unix: ~/.wantcore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "WantCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/WantCore";
#else
    // Unix
    return pathRet / ".wantcore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty want.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override want.conf
